...
variable "memory_floating" {
  description = <<-EOT
    ...
    the VM's actual memory down to this value when idle and grow it back up
    to `memory` when needed.
    - Set to null to default to half of `memory`.
    - Set equal to `memory` to only attach the balloon device
    (for accurate usage reporting) without any real dynamic range.
    - Set to 0 to disable ballooning entirely (fixed allocation).
  EOT
  type        = number
  default     = null
}
